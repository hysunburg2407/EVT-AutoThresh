...
# --- Status area for the file-selection window (permanent, single line) ---
def init_selection_status_widgets(parent):
    """
    Permanent status area for Step-1 (bottom-left):
      - progress bar (determinate) — hidden when idle
      - single-line status label — 'Status: ...'
    Uses pack so it plays nice with the rest of Step-1 which also uses pack.
    """
    global progress_select, status_select_var, lbl_status_select

    # Container for the status area (pack at bottom, full width)
    bar_frame = tk.Frame(parent)
    bar_frame.pack(side="bottom", fill="x", padx=8, pady=(6, 8))

    # Progress bar — determinate, hidden until loading starts
    progress_select = ttk.Progressbar(
        bar_frame, orient="horizontal", mode="determinate", maximum=100
    )
    progress_select.pack(side="top", fill="x")
    progress_select.pack_forget()  # hidden while idle

    # Single-line status label (left aligned)
    status_select_var = tk.StringVar(value="Status: Ready")
    lbl_status_select = tk.Label(bar_frame, textvariable=status_select_var, anchor="w")
    lbl_status_select.pack(side="top", anchor="w")

# >>> STEP2: BACKGROUND LOADER HELPERS
btn_next_global = None  # will be set in open_main_window() so we can enable it after load

def _scan_single_file(fid: int, path: str):
    """
    Worker: read one file and compute the same basic stats you already do in Step-1.
    ...
    try:
        ...
        # --- mirror your existing cleaning ---
        _df["date"] = pd.to_datetime(_df["date"], errors="coerce")
        _df["value"] = pd.to_numeric(_df["value"], errors="coerce")
        
        # --- SKIP RULES (checked during loading) ---
        # A file should be skipped if: all values are NaN OR all non-NaN values are negative
        is_all_nan = _df["value"].isna().all()
        _non_nan = _df["value"].dropna()
        is_all_negative = (len(_non_nan) > 0) and (_non_nan < 0).all()
        
        if is_all_nan or is_all_negative:
            # Return a payload that marks this file as 'to skip' (no error)
            return True, {
                "skip_reason": "All NaN" if is_all_nan else "All Negative",
                "path": path
            }
        # --- END SKIP RULES ---
        ...
        date_min = _df["date"].min()
        date_max = _df["date"].max()
        vmin = float(_df["value"].min())
        vmax = float(_df["value"].max())
...
def start_background_loader(root):
    """
    Kicks off the thread-pool scan over file_paths and updates:
    - progress bar (visible while loading)
    - status line "Status: Reading done/N — <basename>"
    - enables Next + final status with Errors when finished
    """
    global files_data, file_status
    ...
    # show progress bar + set initial status and disable Next
    if progress_select is not None:
        try:
            progress_select.pack(fill="x")
        except Exception:
            pass
        progress_select["value"] = 0

    if status_select_var is not None:
        status_select_var.set(f"Status: Reading 0/{N} — ")

    if btn_next_global is not None:
        try:
            btn_next_global.config(state=tk.DISABLED)
        except Exception:
            pass

    # dynamic, non-thrashy pool size
    workers = min(max(2, (os.cpu_count() or 4) // 2), 8)
    ...
    def _runner():
        ...
        # optimistic "Pending" markers so other code can see progress
        for fid, path in enumerate(file_paths):
            file_status[fid] = "Pending"

        with ThreadPoolExecutor(max_workers=workers) as ex:
            futures = {ex.submit(_scan_single_file, fid, path): (fid, path)
                       for fid, path in enumerate(file_paths)}

            for fut in as_completed(futures):
                fid, path = futures[fut]
                ok = False
                payload = None
                try:
                    ...
                    ok = False
                    payload = None
                    print(f"[Loader error] {os.path.basename(path)}: {type(e).__name__}: {e}")
                
                if ok and isinstance(payload, dict) and ("skip_reason" in payload):
                    # Mark as skipped; do NOT store in files_data
                    reason = payload["skip_reason"]
                    base = os.path.basename(payload.get("path", path))
                    if reason == "All NaN":
                        skipped_nan.append(base)
                    else:
                        skipped_neg.append(base)
                elif ok and payload is not None:
                    files_data[fid] = payload
                    file_status[fid] = "Processed"
                else:
                ...
                # schedule UI updates on Tk thread
                def _update_ui_step():
                    if progress_select is not None:
                        progress_select["value"] = pct
                    if status_select_var is not None:
                        status_select_var.set(f"Status: Reading {done}/{N} — {base}")
                try:
                    root.after(0, _update_ui_step)
                except Exception:
                    pass

        # all done: finalize UI
        def _finalize():
            # --- Ensure the next window has a DataFrame to work with ---
            try:
                ok_fids = [fid for fid, meta in files_data.items()
                           if isinstance(meta.get("df", None), pd.DataFrame)]
                if ok_fids:
                    global current_file_id, df
                    current_file_id = min(ok_fids)
                    df = files_data[current_file_id]["df"].copy()
            except Exception:
                pass
            
            # --- SUMMARY DIALOG BOX AFTER FILE READING ---
            loaded = len(files_data)
            summary = [f"File reading completed.",
                       f"Loaded {loaded} of {N} files."]
            
            if skipped_nan or skipped_neg:
                summary.append("")
                summary.append("Skipping the following files:")
                if skipped_nan:
                    summary.append("  • All NaNs:")
                    summary.extend([f"     - {name}" for name in skipped_nan])
                if skipped_neg:
                    summary.append("  • All Negative values:")
                    summary.extend([f"     - {name}" for name in skipped_neg])
            
            try:
                messagebox.showinfo("File Reading Summary", "\n".join(summary))
            except Exception:
                pass
            # --- END SUMMARY DIALOG BOX ---

            
            if progress_select is not None:
                try:
                    progress_select.pack_forget()  # hide when idle
                except Exception:
                    pass
            if status_select_var is not None:
                if errors == 0:
                    status_select_var.set(f"Status: Ready — Loaded {N} files")
                else:
                    status_select_var.set(f"Status: Ready — Loaded {N} files (Errors: {errors})")
            if btn_next_global is not None:
                try:
                    btn_next_global.config(state=tk.NORMAL)
                except Exception:
                    pass
        try:
...
def open_main_window():
    ...
    def select_file():
        ...
        # --- STEP2: start async loader & return to keep UI responsive ---
        if not file_paths:
            return
        
        # Kick off background scan (shows progress, updates status, disables/enables Next)
        start_background_loader(root)
        return
        # --- END STEP2 INSERT ---
    ...
    global btn_next_global
    btn_next_global = btn_next
    init_selection_status_widgets(root)
...
def open_nan_handling_window():
    ...
    def handle_nan():
        ...
        # --- Safety: ensure df is a valid DataFrame with a 'value' column ---
        if df is None or not isinstance(df, pd.DataFrame) or "value" not in df.columns:
            messagebox.showerror("No data", "No valid data is loaded yet. Please go back and re-open.")
            return
...
# === Parallel-execution config & shared flags ===
from threading import Event
import queue, platform, multiprocessing

USE_PROCESS_POOL = True  # keep True for real multi-core scaling of CPU work

# Pool sizing (tuneable)
CPU_CORES = (os.cpu_count() or 4)
MAX_IO_WORKERS = min(8, max(4, CPU_CORES))      # threads for disk I/O (reads/writes)
MAX_CPU_WORKERS = max(1, CPU_CORES - 1)         # processes for CPU (analysis/plot renders)

# Submission strategy (big batches avoid thousands of pending futures)
BATCH_SUBMIT_SIZE = 300     # submit this many tasks at once
REFILL_WATERMARK = 50       # when pending < this, submit next batch

# Timeouts / retries
ANALYSIS_TIMEOUT_S = 15 * 60   # per-file soft timeout for CPU analysis
IO_MAX_RETRIES = 2             # transient I/O retries for saves/exports

# Checkpoint suffixes (used for resume/idempotency later)
SAVE_DONE_SUFFIX = ".done.save.json"
ANALYSIS_DONE_SUFFIX = ".done.analysis.json"
PLOT_DONE_SUFFIX = ".done.plot.json"

# Shared state for workers → UI
MESSAGE_QUEUE = queue.Queue()  # workers post small dict events; UI drains and updates progress

# Cancellation flag (thread-safe)
CANCELLED = Event()

# Job registry (lightweight run-time ledger; safe to keep in main thread)
JOB_REGISTRY = {}  # file_id -> {"status": "...", "src": "...", "out": "...", "err": "...", ...}

# Lazy pool placeholders (initialized when the dialog opens; closed on exit)
IO_POOL = None
CPU_POOL = None
...
def open_outlier_window():
    ...
    df.loc[(df["value"] >= 0) & (df.index.isin(top_n_values.index)), "Flag"] = "Potential Outlier (User Review)"
    ...
    # --- Helper: mark the currently selected file as skipped (all NaN) ---
    def _mark_current_file_skipped_all_nan():
        try:
            # use global maps already in your app
            base = None
            if isinstance(current_file_id, int) and current_file_id in files_data:
                # tag files_data for this file
                files_data[current_file_id]["skipped_all_nan"] = True
                # update status map (if present)
                try:
                    file_status[current_file_id] = "Skipped (All NaN)"
                except Exception:
                    pass
                # best-effort basename for message
                base = os.path.basename(files_data[current_file_id].get("path", "")) \
                       if isinstance(files_data[current_file_id], dict) else None
            return base
        except Exception:
            return None
    
    def _mark_current_file_skipped_all_negative():
        try:
            base = None
            if isinstance(current_file_id, int) and current_file_id in files_data:
                files_data[current_file_id]["skipped_all_negative"] = True
                try:
                    file_status[current_file_id] = "Skipped (All Negative)"
                except Exception:
                    pass
                base = os.path.basename(files_data[current_file_id].get("path", "")) \
                       if isinstance(files_data[current_file_id], dict) else None
    ...
    def apply_nan_handling():
        ...
        # 1) Skip if all values are NaN (no anchors possible)
        if df["value"].isna().all():
            base = _mark_current_file_skipped_all_nan()
            try:
                msg_file = (base or "this file")
                messagebox.showwarning(
                    "Skipped: all NaNs",
                    f"Skipping {msg_file} because the value column is all NaNs after cleaning."
                )
            except Exception:
                pass
            return
        
        # 2) Skip if all *non-NaN* values are negative (e.g., all -999)
        _non_nan = df["value"].dropna()
        if len(_non_nan) > 0 and (_non_nan < 0).all():
            base = _mark_current_file_skipped_all_negative()
            try:
                msg_file = (base or "this file")
                messagebox.showwarning(
                    "Skipped: all negative values",
                    f"Skipping {msg_file} because all non-NaN values are negative (e.g., -999)."
                )
            except Exception:
                pass
            return
        
        # 3) Proceed with normal flow: treat negatives as missing so chosen NaN-methods can act
    ...
    def process_negative_values_only():
        ...
        df["value"] = pd.to_numeric(df["value"], errors='coerce')
        invalid_mask = (df["value"] < 0) | (df["value"].isna())
        invalid_value_indexes = df[invalid_mask].index.tolist()
...
def open_save_data_window():
    ...
    def save_all_cleaned():
        ...
        # --- Parallel I/O write using ThreadPoolExecutor ---
        from concurrent.futures import ThreadPoolExecutor, as_completed

        total = 0
        failures = []

        def _save_one(fid, src_path):
            """Worker: save one cleaned CSV file."""
            try:
                if fid not in files_data or "df" not in files_data[fid]:
                    return False, src_path, "no dataframe"
                ...
                # Skip if already exists with same size (idempotent)
        ...
        # Decide number of I/O workers
        workers = min(MAX_IO_WORKERS, max(2, (os.cpu_count() or 4) // 2))

        with ThreadPoolExecutor(max_workers=workers) as ex:
            futures = {ex.submit(_save_one, fid, src): (fid, src) for fid, src in enumerate(file_paths)}
            for fut in as_completed(futures):
                ok, src_path, err = (False, futures[fut][1], "unknown")
                try:
                    ok, src_path, err = fut.result()
                except Exception as e:
                    err = str(e)
                    ok = False
                if ok and (err is None or err == "skipped"):
                    total += 1
                else:
                    failures.append((os.path.basename(src_path), err))
...
# --- Two-stage workers: reader (thread) + analyzer (process) ---
def _reader_task(src_path):
    """
    Lightweight thread-safe reader: loads one cleaned CSV and validates.
    Returns (df_local, basename, error_message_or_None).
    """
    try:
        df_local = pd.read_csv(src_path)

        cols = list(df_local.columns)
        if len(cols) < 2 or cols[0] != "date" or cols[1] != "value":
            return None, os.path.basename(src_path), "Invalid headers"
        ...
        # Skip rules
        is_all_nan = df_local["value"].isna().all()
        _non_nan = df_local["value"].dropna()
        is_all_negative = (len(_non_nan) > 0) and (_non_nan < 0).all()
        if is_all_nan:
            return None, os.path.basename(src_path), "SKIP: All NaN"
        if is_all_negative:
            return None, os.path.basename(src_path), "SKIP: All Negative"

        return df_local, os.path.basename(src_path), None
    except Exception as e:
        return None, os.path.basename(src_path), f"{type(e).__name__}: {e}"

def _analyze_task(csv_path, fid):
    """
    CPU-heavy worker: reads its own temp CSV and runs perform_analysis().
    Returns (fid, results_df, error or None).
    """
    try:
        df_local = pd.read_csv(csv_path)
        res = perform_analysis(df_local)
        return fid, res, None
    except Exception as e:
        return fid, None, f"{type(e).__name__}: {e}"
...
def open_processing_dialog(files_data):
    ...
    # ---- UI: non-blocking Toplevel ----
    processing_dialog = tk.Toplevel()  # (was tk.Tk())
    ...
    progress = ttk.Progressbar(processing_dialog, orient="horizontal",
                               mode="determinate", maximum=len(file_paths), length=360)
    progress.pack(pady=10)

    processing_dialog.update_idletasks()  # show immediately

    # ---- shared state (unchanged outputs) ----
    ...
    # ---- WORKER: same pipeline as before, just moved off the UI thread ----
    def _worker():
        # NOTE: we re-use your exact logic; we only replace direct UI updates
        global IO_POOL, CPU_POOL
        try:
            if IO_POOL is None:
                IO_POOL = ThreadPoolExecutor(max_workers=MAX_IO_WORKERS)
            if USE_PROCESS_POOL and CPU_POOL is None:
                CPU_POOL = multiprocessing.get_context("spawn").Pool(processes=MAX_CPU_WORKERS)
        except Exception as e:
            print(f"[InitPools] Warning: failed to initialize pools — {e}")

        skipped_nan, skipped_neg = [], []
        file_status = {}
        done = 0
        total = len(file_paths)

        def _submit_reader(fid, src_path):
            f = IO_POOL.submit(_reader_task, src_path)
            f.fid = fid
            return f

        reader_futures = [_submit_reader(fid, src) for fid, src in enumerate(file_paths)]

        for fut in as_completed(reader_futures):
            fid = getattr(fut, "fid", None)
            basename = os.path.basename(file_paths[fid]) if fid is not None else "<unknown>"

            try:
                df_local, base, err = fut.result()
            except Exception as e:
                df_local, err = None, f"{type(e).__name__}: {e}"

            if err is None and df_local is not None:
                try:
                    # write temp csv → CPU pool (unchanged)
                    import tempfile
                    with tempfile.NamedTemporaryFile(delete=False, suffix=".csv") as tmp:
                        tmp_path = tmp.name
                    df_local.to_csv(tmp_path, index=False)

                    result = CPU_POOL.apply_async(_analyze_task, args=(tmp_path, fid))
                    fid_r, results_df, err_r = result.get(timeout=ANALYSIS_TIMEOUT_S)
                    ...
                    if err_r is None and results_df is not None:
                        analysis_results_per_file[fid_r] = results_df
                        analysis_file_names[fid_r] = basename
                        file_status[fid_r] = "Processed"
                        uiq.put(("status", f"Processing {done + 1}/{total}: {basename} ✓"))
                    else:
                        file_status[fid_r] = "Error"
                        uiq.put(("status", f"Processing {done + 1}/{total}: {basename} — ERROR"))
                        print(f"[Analysis error] {basename}: {err_r}")

                except Exception as e:
                    file_status[fid] = "Error"
                    print(f"[CPU error] {basename}: {e}")
                    uiq.put(("status", f"Processing {done + 1}/{total}: {basename} — CPU ERROR"))
            else:
                # handle explicit SKIPs from reader
                if isinstance(err, str) and err.startswith("SKIP:"):
                    reason = err.split(":", 1)[1].strip()
                    if reason == "All NaN":
                        skipped_nan.append(basename)
                        file_status[fid] = "Skipped (All NaN)"
                    elif reason == "All Negative":
                        skipped_neg.append(basename)
                        file_status[fid] = "Skipped (All Negative)"
                    uiq.put(("status", f"Processing {done + 1}/{total}: {basename} — {reason}"))
                else:
                    file_status[fid] = "Error"
                    uiq.put(("status", f"Processing {done + 1}/{total}: {basename} — ERROR"))
                    print(f"[Read error] {basename}: {err}")

            done += 1
            uiq.put(("progress", done))

        # finalize & summary (identical text as your current code)
        loaded_count = len(analysis_results_per_file)
        selected_count = len(file_paths)
        summary = [f"File reading completed.",
                   f"Loaded {loaded_count} of {selected_count} files."]
    ...
    # ---- UI pump: consume worker messages safely on the main thread ----
    def _drain():
        try:
            while True:
                kind, payload = uiq.get_nowait()
                if kind == "status":
                    lbl_message.config(text=payload)
                elif kind == "progress":
                    progress["value"] = payload
                elif kind == "done":
                    # show summary, close dialog, continue flow
                    try:
                        messagebox.showinfo("Load summary", payload)
                    except Exception:
                        pass
            ...
            pass
        processing_dialog.after(75, _drain)  # keep polling lightly

    _drain()  # start polling
...
def open_next_step():
    ...
    def render_for_file(fid: int):
        ...
        # --- Fallback: if XX not found but YY exists, set XX to the threshold just before YY ---
        if XX is None and YY is not None:
            thr = df[threshold_col].values
            # assumes thr is ascending; find index of the left bin edge before YY
            idx_left = np.searchsorted(thr, YY, side="right") - 1
            if idx_left >= 0:
                XX = thr[idx_left]
...
if __name__ == "__main__":
    multiprocessing.freeze_support()  # ✅ Required for Windows spawn mode
    open_main_window()
